...
image = pygame.image.load('data\image-800x600.jpg')
...
while is_running:
    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.MOUSEWHEEL:
            if (pos1 + (event.y * -1) > 0 or event.y > 0) and (pos1 - (event.y * -1) < SCREEN_HEIGHT or event.y > 0):
                pos1 += event.y * 20 * -1
                print(event.y)
            percentage_y = pos1 / screen_rect.height
            camera_rect.y = percentage_y * (image_rect.height - screen_rect.height)
